...
class _ChickenHouseScreenState extends State<ChickenHouseScreen> {
  ...
  void initState() {
    ...
    widget.token  == null ? fetchChickenHouseDataFromLocal(context, date: dateController.text) : 
  ...
  Widget build(BuildContext context) {
    ...
    List<ChickenHouseData> chickenHouseDataListWhenTokenIsAvailableList = Provider.of<ChickenHouseDataProvider>(context, listen: true).chickenHouseDataListWhenTokenIsAvailableList;
      ...
      child: Scaffold(
        appBar: AppBar(
          ...
          actions: [
            ...
            if(widget.token != null && chickenHouseDataListWhenTokenIsAvailableList.isNotEmpty)...[
              isSyncing ? const BallPulseIndicator() :
              IconButton(
                onPressed: () async {
                  setState(() {
                    isSyncing = true;
                  ...
                  await Provider.of<ChickenHouseDataProvider>(
                    context, 
                    listen: false
                  ).uploadData(context, token: widget.token!, date: dateController.text).then(
                    (_){
                      setState(() {
                        isSyncing = false;
                      });
              ...
              child: SingleChildScrollView(
                ...
                child: Column(
                  ...
                  children: <Widget>[
                      ...
                      child: SizedBox(
                        ...
                        child: GridView.builder(
                          ...
                          itemBuilder: (BuildContext context, int index) {
                            ...
                            final isDisabled =  chickenHouseDataListWhenTokenIsAvailableList.any((data) => data.item == category.name) || chickenHouseDataList.any((data) => data.item == category.name);
                    ...
                    isFetchingChickenHouseData ? const BallPulseIndicator() : chickenHouseDataList.isEmpty ? SizedBox(
                      height: chickenHouseDataListWhenTokenIsAvailableList.isEmpty ? MediaQuery.of(context).size.height * 0.5 : MediaQuery.of(context).size.height * 0.2,
                      width: MediaQuery.of(context).size.width,
                      child: Column(
                        mainAxisAlignment: MainAxisAlignment.start,
                  ...
                  : ListView.builder(
                    ...
                    if(chickenHouseDataListWhenTokenIsAvailableList.isNotEmpty)...[
          
                      Text(
                        dateController.text == DateTime.now().toString().split(' ')[0] ? "Today's Data, \nLocally Stored" : "Chicken House Data on ${dateController.text}, \nLocally Stored",
                        style: Theme.of(context).textTheme.bodyLarge,
                      ),
                      const SizedBox(
                        height: 21,
                      ...
                      ListView.builder(
                        ...
                        shrinkWrap: true,
                        reverse: true,
                        itemCount: chickenHouseDataListWhenTokenIsAvailableList.length,
                        itemBuilder: (BuildContext context, int index) {
                          var chickenHouseData = chickenHouseDataListWhenTokenIsAvailableList[index];
                          return widget.token == null ?
                          ChickenHouseDataCard(
                            chickenHouseData: chickenHouseData,
                            date: dateController.text,
